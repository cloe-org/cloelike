...
class EuclidLikelihood_GC_Cls:
    ...
    def _prepare(self):
        ...
        if self.settings['n_ell_bins'] < len(self.data['ells']):
            ...
            self._bin_data(self.data['cells'], self.data['ells'],
                           self.settings['n_ell_bins'])
            self._bin_mixmat()
    ...
    def _flatten_data_vector_and_mask(self):
        ...
        self.data_vector = np.array(
            [self.data['cells'][key] for key in self.GG_keys]).flatten()
    ...
    def get_theory_vector(self, parameters: dict):
        ...
        pos = PositionsTracer(
            nlp, self.data['dndz_pos'], self.zs, galaxy_bias_model='poly',
        ...
        theory_vector = np.array(
            [cell_all_th[key] for key in self.GG_keys]).flatten()
    ...
    def loglike(self, parameters: dict):
        ...
        chi2 = np.dot(
            np.dot(diff, self.inverse_masked_covariance_matrix), diff)
