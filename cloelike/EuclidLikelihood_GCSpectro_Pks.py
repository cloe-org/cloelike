...
class EuclidLikelihoodGCspectroPks:
    ...
    def __init__(self, data: dict, settings: dict,
        ...
        if self.AM_priors:
            unmatched_z = [z for z in AM_priors.keys()
            ...
            self.AM_par_to_diag = {
                ...
                'NP0': ['noise_k0'], 'NP20': ['noise_k2'],
                'NP22': ['noise_k2mu2']
            }
            self.AM_diagrams = [term for values in self.AM_par_to_diag.values()
                                for term in values]
            ...
            for z in self.AM_priors.keys():
                ...
                AM_means.extend([self.AM_priors[z][par][0]
                                 for par in self.AM_priors[z].keys()])
                AM_sigmas.extend([self.AM_priors[z][par][1]
                                  for par in self.AM_priors[z].keys()])
    ...
    def get_theory_vector_AM(self, parameters: dict, term_list: dict):
        ...
        for i,z in enumerate(self.redshifts):
            ...
            if z in self.AM_params.keys():
                ...
                Nk = sum(len(self.data['GCspectro'][z][f'pk{ell}'])
                         for ell in self.ells)
    ...
    def _mask_theory_vector_AM(self):
        ...
        self.masked_theory_vector_AM_reduced = \
            self.theory_vector_AM_reduced[:, self.masking_vector]
    ...
    def loglike_AM(self, parameters: dict):
        ...
        term_list = {
            z: [value for key in self.AM_params[z]
        ...
        self.theory_vector, self.theory_vector_AM = \
            self.get_theory_vector_AM(parameters, term_list)
        ...
        for z in self.redshifts:
            ...
            if z not in self.AM_params or len(self.AM_params[z]) == 0:
                matrix_reduced_z = np.zeros(
                    (0, self.theory_vector_AM[z].shape[1]))
            # Otherwise, we correctly recombine the terms that requires it
            else:
                indices_groups = [[term_list[z].index(term)
        ...
        diff = self.masked_theory_vector - self.masked_data_vector
        F0 = np.einsum('i,ij,j->', diff, self.inverse_masked_covariance_matrix,
                       diff) + np.sum((self.AM_means / self.AM_sigmas)**2)
        F1i = -np.einsum('ij,jk,k->i', self.masked_theory_vector_AM_reduced,
                         self.inverse_masked_covariance_matrix,
                         diff) + self.AM_means / self.AM_sigmas**2
        F2ij = (np.einsum('ik,kp,jp->ij', self.masked_theory_vector_AM_reduced,
                          self.inverse_masked_covariance_matrix,
                          self.masked_theory_vector_AM_reduced)
