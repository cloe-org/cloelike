...
# --- Covariance slicing helper ---
def get_cov_slice(probe, full_cov):
    n_wl = (21+21)*32
    n_gc = 36*32
    if probe == 'WL':
        return full_cov[:n_wl, :n_wl]
    elif probe == 'GC':
        return full_cov[n_wl+n_gc:, n_wl+n_gc:]
    elif probe == '2x2pt':
        return full_cov[n_wl:, n_wl:]
    elif probe == '3x2pt':
        return full_cov
...
# --- Test: Likelihood returns finite and non-positive loglike for default parameters ---
@pytest.mark.parametrize("LikeCls, ell_key, probe, pos, she", [
    (EuclidLikelihood_3x2pt_Cls, ('POS', 'POS', 1, 1), '3x2pt', True, True),
    (EuclidLikelihood_2x2pt_Cls, ('POS', 'POS', 1, 1), '2x2pt', True, True),
    (EuclidLikelihood_WL_Cls,    ('SHE', 'SHE', 1, 1), 'WL',    False, True),
    (EuclidLikelihood_GC_Cls,    ('POS', 'POS', 1, 1), 'GC',    True, False),
])
def test_likelihood_negative_or_zero(data_setup, LikeCls, ell_key, probe, pos, she):
    cov = get_cov_slice(probe, data_setup['full_cov'])
    data = build_data(ell_key, cov, data_setup, include_pos=pos, include_she=she)
...
# --- Test: Likelihood changes when cosmological parameters are changed ---
@pytest.mark.parametrize("LikeCls, ell_key, probe, pos, she", [
    (EuclidLikelihood_3x2pt_Cls, ('POS', 'POS', 1, 1), '3x2pt', True, True),
    (EuclidLikelihood_2x2pt_Cls, ('POS', 'POS', 1, 1), '2x2pt', True, True),
    (EuclidLikelihood_WL_Cls,    ('SHE', 'SHE', 1, 1), 'WL',    False, True),
...
def test_likelihood_changes_with_parameters(data_setup, LikeCls, ell_key, probe, pos, she):
    cov = get_cov_slice(probe, data_setup['full_cov'])
    data = build_data(ell_key, cov, data_setup, include_pos=pos, include_she=she)
    settings = build_settings(data_setup)
    like = LikeCls(
    ...
    test_pars = default_pars.copy(); test_pars['H0'] += 5
    logl_changed = like.loglike(test_pars)
    assert logl_default != logl_changed

# --- Test: Likelihood raises exception for unphysical/bad parameters ---
@pytest.mark.parametrize("LikeCls, ell_key, probe, pos, she", [
    (EuclidLikelihood_3x2pt_Cls, ('POS', 'POS', 1, 1), '3x2pt', True, True),
    (EuclidLikelihood_2x2pt_Cls, ('POS', 'POS', 1, 1), '2x2pt', True, True),
    (EuclidLikelihood_WL_Cls,    ('SHE', 'SHE', 1, 1), 'WL',    False, True),
...
def test_likelihood_handles_bad_parameters(data_setup, LikeCls, ell_key, probe, pos, she):
    cov = get_cov_slice(probe, data_setup['full_cov'])
    data = build_data(ell_key, cov, data_setup, include_pos=pos, include_she=she)
    settings = build_settings(data_setup)
    like = LikeCls(
    ...
    bad_pars = default_pars.copy(); bad_pars['H0'] = -100
