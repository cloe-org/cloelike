...
def data_setup(tmp_path_factory):
    ...
    for filename, url in urls.items():
        ...
        with fits.open(tmpdir / filename) as hdul:
            ...
            fiducial_cosmo = {
                key_cloe: hdr[key] for key_cloe, key in
                zip(['H0', 'Omega_m0', 'Omega_b0'],['H0', 'OMEGA_M', 'OMEGA_B'])}

            fiducial_cosmo['Omega_cdm0'] = fiducial_cosmo.pop('Omega_m0') - \
                fiducial_cosmo['Omega_b0']

            for key,val in zip(['Omega_k0', 'As', 'ns', 'mnu','w0','wa','gamma_MG'],
                               [0., 2.1e-9, 0.96, 0., -1., 0., 0.545]):
