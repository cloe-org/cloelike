...
class EuclidLikelihood_3x2ptPlusGCspectro_ClsPlusPls:
    def __init__(
        ...
        self.scale_cuts = {
            probe: settings[probe]["scale_cuts"] for probe in ["3x2pt", "GCspectro"]
        }
    ...
    def _prepare(self):
        ...
        """
        if self.settings["3x2pt"]["n_ell_bins"] < len(self.data["3x2pt"]["ells"]):
            self.rebin = True
            self.data["3x2pt"]["cells_unbin"] = self.data["3x2pt"]["cells"]
            self.data["3x2pt"]["ells_unbin"] = self.data["3x2pt"]["ells"]
            self._bin_data(
                self.data["3x2pt"]["cells"],
                self.data["3x2pt"]["ells"],
    ...
    def _bin_mixmat(self):
        ...
        for k in self.mixmat["3x2pt"].keys():
            new_array = np.tensordot(
                self.weight_mat, self.mixmat["3x2pt"][k], axes=([1], [-2])
            )
            if k[:2] == ("SHE", "SHE"):
    ...
    def _flatten_data_vector_and_mask(self):
        ...
        self.data_vector["3x2pt"] = np.concatenate(
            [
                np.transpose(
                    [self.data["3x2pt"]["cells"][key][:2] for key in self.WL_keys],
                    axes=(1, 0, 2),
                ).flatten(),
                np.array(
                    [self.data["3x2pt"]["cells"][key][:1] for key in self.GGL_keys]
                ).flatten(),
                np.array(
                    [self.data["3x2pt"]["cells"][key] for key in self.GG_keys]
                ).flatten(),
            ]
        )
        ...
        self.masking_vector["3x2pt"] = np.concatenate(
            [
                ...
                np.transpose(
                    [
                        self._masking(
                            self.data["3x2pt"]["ells"], self.scale_cuts["3x2pt"][key]
                        )
        ...
        self.masked_data_vector["3x2pt"] = self.data_vector["3x2pt"][
            self.masking_vector["3x2pt"]
        ]

        self.masked_data_vector["GCspectro"] = self.data_vector["GCspectro"][
    ...
    def _mask_covariance_and_invert(self):
        ...
        self.inverse_masked_covariance_matrix["3x2pt"] = np.linalg.inv(
            self.covariance_matrix["3x2pt"][self.masking_vector["3x2pt"]][
                :, self.masking_vector["3x2pt"]
            ]
        )

        self.inverse_masked_covariance_matrix["GCspectro"] = np.linalg.inv(
            self.covariance_matrix["GCspectro"][self.masking_vector["GCspectro"]][
                :, self.masking_vector["GCspectro"]
            ]
        )
    ...
    def _mask_theory_vector(self):
        ...
        self.masked_theory_vector["3x2pt"] = self.theory_vector["3x2pt"][
            self.masking_vector["3x2pt"]
        ]
        self.masked_theory_vector["GCspectro"] = self.theory_vector["GCspectro"][
            self.masking_vector["GCspectro"]
        ]
    ...
    def loglike(self, parameters: dict):
        ...
        chi2_GCspectro = np.dot(
            np.dot(diff_GCspectro, self.inverse_masked_covariance_matrix["GCspectro"]),
            diff_GCspectro,
        )
